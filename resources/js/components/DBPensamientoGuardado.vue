...
<script>
	export default{
		...
		mounted(){ 
			console.log('Template de Display DB OK')
		},
		methods : {
			eliminar(){
				axios.delete("/pensamientos/"+this.pensamiento_local.id)
				.then((response)=>
					 {
					 	this.$emit('borrar')
					 }
			...
			actualizar(){
				const parametros_a_enviar={
					'desc':this.pensamiento_local.desc
				}
				axios.put("/pensamientos/"+this.pensamiento_local.id,parametros_a_enviar).then((response)=>
					 {
